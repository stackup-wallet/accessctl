...
A policy can enable rate limits on a `UserOperation` via the `minimumInterval` field. This is a unix timestamp to specify how long a signer must wait between consecutive `UserOperations`.
